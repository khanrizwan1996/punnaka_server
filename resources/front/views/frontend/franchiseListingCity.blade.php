            ...
            <div class="row">
                <div class="col-lg-12 col-md-12  row">
                    <div class="utf_dashboard_list_box margin-top-0">
                        <h4 class="gray"> {{ str_replace('-', ' ', Route::input('city')) ; }} Franchise List</h4>
                    </div>
                    <div class="listing_filter_block">
                        <div class="col-md-12 col-xs-12">
                            <div class="sort-by">
                                <div class="col-md-12 col-xs-12 row">
                                @foreach ($franchiseCityData as $franchiseCityRow)
                                        <div class="col-md-6" style="margin-top: 30px;">
                                            <a class="utf_common_button" style="background-color: #87ceeb; color:white; font-weight:bold" href="#">{{$franchiseCityRow->f_main_cat}}</a>

                                            @php
                                            $franchiseCityBusinessData = $franchiseObj->franchiseCityBusinessList($franchiseCityRow->f_city,$franchiseCityRow->f_sub_cat);
                                            @endphp

                                        @foreach ($franchiseCityBusinessData as $franchiseCityBusinessResult)
                                        @php
                                            $franchiseCityName = strtolower(str_replace(' ', '-', $franchiseCityBusinessResult->f_city));
                                            $franchiseSubCategoryName = strtolower(str_replace(' ', '-', $franchiseCityBusinessResult->f_sub_cat));
                                        @endphp
                                            <span style="text-decoration: underline;">
                                            <a href="{{url('franchise-category/'.$franchiseCityName.'/'.$franchiseSubCategoryName)}}"> {{$franchiseCityBusinessResult->f_sub_cat}}</a>
                                            </span> &emsp;
                                        @endforeach <br>
                                        </div>
                                    @endforeach
