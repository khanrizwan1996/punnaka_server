        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-8 listing_grid_item">
                    <div class="row">
                        @foreach ($couponLisitngCategoryArray as $couponLisitngCategoryData)
                            @php
                            $couponImagePathObj = new App\Http\Controllers\frontend\couponController();
                            $couponImagePath = $couponImagePathObj->GetCouponImage($couponLisitngCategoryData['coupon_id']);

                            $couponCityName = str_replace(' ', '-', $couponLisitngCategoryData['coupon_city']);
                            $couponCategoryName = str_replace(' ', '-', $couponLisitngCategoryData['coupon_sub_category']);
                            $couponName = str_replace(' ', '-', $couponLisitngCategoryData['coupon_title']);
                            @endphp

                            <div class="col-lg-6 col-md-12">
                                <a href="{{url('coupon-detail/'.$couponCityName.'/'.$couponCategoryName.'/'.$couponName)}}"  data-marker-id="1">
                                    <div class="utf_listing_item">
                                      <img src="{{ url('custom-images/coupons/'. $couponImagePath) }}"
                                            alt="">

                                        <div class="utf_listing_item_content">
                                             <div class="utf_listing_prige_block">
                                                <span class="utf_meta_listing_price"><i class="fa fa-phone"></i>
                                                    {{ $couponLisitngCategoryData['coupon_contact'] }}</span>
                                            </div>
                                            <h3>{{ $couponLisitngCategoryData['coupon_title'] }}</h3>
                                            <span><i class="fa fa-map-marker"></i> Location (State) :
                                                {{ Str::substr($couponLisitngCategoryData['coupon_state'], 0, 10) }}</span>
                                            <span><i class="fa fa-phone"></i> Location (City) :
                                                {{ Str::substr($couponLisitngCategoryData['coupon_city'], 0, 10) }}</span>
                                        </div>
                                    </div>
                                </a>
                            </div>
                ...
                <div class="col-lg-4 col-md-4">
                    <div class="sidebar">
                        <div class="utf_box_widget margin-top-35 margin-bottom-75">
                            @php
                                $getLatestCouponListingObj = new App\Http\Controllers\frontend\couponController();
                                $getLatestCouponListingArray = $getLatestCouponListingObj->getLatestCouponListing(Route::input('city'));
                            @endphp
                            <h3><i class="sl sl-icon-folder-alt"></i>{{ str_replace(' ', '-', Route::input('city') )}} Categories</h3>
                            <ul class="utf_listing_detail_sidebar">
                                @foreach ($getLatestCouponListingArray as $getLatestCouponListingRow)

                                    @php
                                        $latestCouponCityName = str_replace(' ', '-', $getLatestCouponListingRow['coupon_city']);
                                        $latestCouponCategoryName = str_replace(' ', '-', $getLatestCouponListingRow['coupon_sub_category']);
                                    @endphp
                                    <li><i class="fa fa-angle-double-right"></i> <a href="{{ url('coupon-category/'.$latestCouponCityName.'/'.$latestCouponCategoryName) }}">{{ $getLatestCouponListingRow->coupon_title }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
