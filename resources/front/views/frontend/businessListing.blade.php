                    ...
                    <div class="col-md-12">
                        ...
                        <nav id="breadcrumbs">
                            <ul>
                                ...
                                <li>{{ str_replace('-', ' ', Route::input('country')) }}</li>
                            </ul>
                        </nav>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12  row">
                    <div class="utf_dashboard_list_box margin-top-0">
                        <h4 class="gray"> {{ str_replace('-', ' ', Route::input('country')) }} Business List</h4>
                    </div>
                    <div class="listing_filter_block">
                        <div class="col-md-12 col-xs-12">
                            <div class="sort-by">
                                <div class="col-md-12 col-xs-12 row">
                                    @foreach ($businessDataArray as $businessDataArrayResult)
                                        <div class="col-md-4" style="margin-top: 30px;">
                                            <a class="utf_common_button"
                                                style="background-color: #87ceeb; color:white; font-weight:bold"
                                                href="#">{{ $businessDataArrayResult->bus_state }}</a>

                                            @php
                                                $businessListingStateObj = new App\Http\Controllers\frontend\businessListingController();
                                                $businessListingStateArray = $businessListingStateObj->businessLisitngCity($businessDataArrayResult->bus_country, $businessDataArrayResult->bus_state);
                                            @endphp
                                            @foreach ($businessListingStateArray as $businessStateDataArrayResult)
                                                @php
                                                    $businessCityName = str_replace(' ', '-', $businessStateDataArrayResult->bus_city);
                                                @endphp
                                                <span style="text-decoration: underline;">
                                                    <a href="{{ url('city/' . $businessCityName) }}">
                                                        {{ $businessStateDataArrayResult->bus_city }}</a>
