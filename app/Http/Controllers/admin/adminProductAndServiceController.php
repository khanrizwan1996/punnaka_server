...
class adminProductAndServiceController extends Controller
{
    public function productAndServiceListView(){
        $productAndServiceData = ProductService::orderBy('ps_id', 'DESC')->get();
        return view('admin.productAndServiceList',['productAndServiceData'=>$productAndServiceData]);
    }
    public function productAndServiceAddView(){
        ...
        return view('admin.productAndServiceAdd',['mainCategoryData'=>$mainCategoryData,'UserData'=>$UserData]);
    }
     public function productAndServiceEditView(Request $request){
        $UserData = User::where(['status'=>STATUS_ACTIVE])->orderBy('id', 'DESC')->get();
        $getProductServiceData = ProductService::where('ps_id',$request->id)->first();
        $mainCategoryData = CategoryMain::where(['cat_main_status'=>STATUS_ACTIVE])->orderBy('cat_main_id', 'DESC')->get();
        return view('admin.productAndServiceEdit',['getProductServiceData'=>$getProductServiceData,'mainCategoryData'=>$mainCategoryData,'UserData'=>$UserData]);
    }
     ...
     public function productAndServiceStore(Request $request){
       ...
       //dd(DB::getQueryLog());203
        ...
        $couponInsertData = ProductService::create([
            ...
            'ps_admin_comment' => $request->ps_admin_comment,
            'ps_added_time' => CURRENT_DATE_TIME
        ]);
        if($couponInsertData){
            return redirect('/admin/productAndServiceAdd')->with('message',MSG_ADD_SUCCESS);
        }else{
            return redirect('/admin/productAndServiceAdd')->with('message',MSG_ADD_ERROR);
        }
    }
    ...
    public function productAndServiceUpdate(Request $request){
        ...
        if($couponUpdateData){
            return redirect('admin/productAndServiceEdit/'.$request->id)->with('message',MSG_UPDATE_SUCCESS);
        }else{
            return redirect('admin/productAndServiceEdit/'.$request->id)->with('message',MSG_UPDATE_ERROR);
        }
    }
}
