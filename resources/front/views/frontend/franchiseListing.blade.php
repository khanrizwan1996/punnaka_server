            ...
            <div class="row">
                <div class="col-lg-12 col-md-12  row">
                    <div class="utf_dashboard_list_box margin-top-0">
                        <h4 class="gray"> {{ str_replace('-', ' ', Route::input('country')) }} Franchise List</h4>
                    </div>
                    <div class="listing_filter_block">
                        <div class="col-md-12 col-xs-12">
                            <div class="sort-by">
                                <div class="col-md-12 col-xs-12 row">
                                    @foreach ($franchiseDataArray as $franchiseResult)
                                        <div class="col-md-4" style="margin-top: 30px;">
                                            <a class="utf_common_button"
                                                style="background-color: #87ceeb; color:white; font-weight:bold"
                                                href="#">{{ $franchiseResult->f_state }}</a>

                                            @php
                                                $franchiseStateObj = new App\Http\Controllers\frontend\franchiseController();
                                                $franchiseStateArray = $franchiseStateObj->franchiseLisitngState($franchiseResult->f_country, $franchiseResult->f_state);
                                            @endphp
                                            @foreach ($franchiseStateArray as $franchiseStateResult)
                                                @php
                                                    $franchiseCityName = strtolower(str_replace(' ', '-', $franchiseStateResult->f_city));
                                                @endphp
                                                <span style="text-decoration: underline;">
                                                    <a href="{{ url('franchise-city/' . $franchiseCityName) }}">
                                                        {{ $franchiseStateResult->f_city }}</a>
