...
@section('main-container')
  ...
  <div class="container" id="scrollTopId">
      ...
      <div class="row margin-bottom-60">
          @foreach ($blogData as $blogRow)
              @php
                  $newBlogTitle = str_replace(' ', '-', $blogRow['blog_title']);
              @endphp
              <div class="col-md-4 col-sm-6 col-xs-12 paginDiv">
                <img src="{{ url('custom-images/blogs/'. $blogRow['blog_image'] . '') }}" alt="" class="img-box">
                 <div class="utf_img_content_box visible" style="width: 90%;">
                    {{-- <h4> {{ $blogRow['blog_title'] }} </h4> --}}
                    <a href="{{ url('detail-blog/' . $newBlogTitle) }}"><span style="background: #3fb4e4">View Details</span></a>
                </div>
                  {{-- <a href="{{ url('detail-blog/' . $newBlogTitle) }}" class="img-box" data-background-image="{{ url('custom-images/blogs/'. $blogRow['blog_image'] . '') }}">
                      <div class="utf_img_content_box visible">
                          <h4> {{ Str::substr($blogRow['blog_title'], 0, 27) }} </h4>
                      ...
                      <div class="row">
                          ...
                          <div class="col-xl-12 col-md-12 col-sm-12">
                              <div class="product-search-count-bottom">
                                  <h5>Showing Blog {{ count($blogData) }} Result</h5>
                              </div>
                          </div>
                      </div>
                  </div>
