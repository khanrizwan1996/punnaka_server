    ...
    @section('main-container')
        <div id="titlebar" class="gradient">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <h2>{{ Route::input('country'); }} Coupon List</h2>
                        <nav id="breadcrumbs">
                            <ul>
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12  row">
                    <div class="utf_dashboard_list_box margin-top-0">
                        <h4 class="gray"> {{ str_replace('-', ' ', Route::input('city')) ; }} Coupon List</h4>
                    </div>
                    <div class="listing_filter_block">
                        <div class="col-md-12 col-xs-12">
                            <div class="sort-by">
                                <div class="col-md-12 col-xs-12 row">
                                @foreach ($couponCityArray as $couponCityRow)
                                        <div class="col-md-4">
                                            <a class="utf_common_button" style="background-color: #87ceeb; color:white; font-weight:bold" href="#">{{$couponCityRow->coupon_main_category}}</a>
                                            <br>

                                            @php
                                            $couponCitySubCategoryDataObj = new App\Http\Controllers\frontend\couponController();
                                            $couponCitySubCategoryDataArray = $couponCitySubCategoryDataObj->couponCitySubCategoryData($couponCityRow->coupon_city,$couponCityRow->coupon_sub_category);
                                            @endphp

                                        @foreach ($couponCitySubCategoryDataArray as $couponCitySubCategoryDataRow)
                                        @php
                                            $couponCityName = str_replace(' ', '-', $couponCitySubCategoryDataRow->coupon_city);
                                            $couponSubCategoryName = str_replace(' ', '-', $couponCitySubCategoryDataRow->coupon_sub_category);
                                        @endphp
                                            <span style="text-decoration: underline;">
                                            <a href="{{url('coupon-category/'.$couponCityName.'/'.$couponSubCategoryName)}}"> {{$couponCitySubCategoryDataRow->coupon_sub_category}}</a>
                                            </span> &emsp;
                                        @endforeach
                                        </div>
                                    @endforeach
