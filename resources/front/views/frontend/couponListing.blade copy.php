            ...
            <div class="row">
                <div class="col-lg-12 col-md-12  row">
                    <div class="utf_dashboard_list_box margin-top-0">
                        <h4 class="gray"> {{ Route::input('country') }} - Coupon List</h4>
                    </div>
                    <div class="listing_filter_block">
                        <div class="col-md-12 col-xs-12">
                            <div class="sort-by">
                                <div class="col-md-12 col-xs-12 row">
                                    @foreach ($couponCountryDataArray as $couponCountryRow)
                                        <div class="col-md-4">
                                            <a class="utf_common_button"
                                                style="background-color: #87ceeb; color:white; font-weight:bold"
                                                href="#">{{ $couponCountryRow->coupon_state }}</a>
                                            <br>
                                            @php
                                            $couponCityObj = new App\Http\Controllers\frontend\couponController();
                                            $couponCityArray = $couponCityObj->couponCity($couponCountryRow->coupon_country, $couponCountryRow->coupon_state);
                                            @endphp
                                            @foreach ($couponCityArray as $couponCityRow)
                                                @php
                                                    $couponCityName = str_replace(' ', '-', $couponCityRow->coupon_city);
                                                @endphp
                                                <span style="text-decoration: underline;">
                                                    <a href="{{ url('coupon-city/' . $couponCityName) }}">
                                                        {{ $couponCityRow->coupon_city }}</a>
                                                </span> &emsp;
                                            @endforeach
                                        </div>
                                    @endforeach
